...
static bool check_string_obj(PyObject *utf8)
{
...
static bool check_string_obj(PyObject *str)
{
...
static unsigned long convert_function_addr(PyObject *frame)
{
	...
	if (!strcmp(str_name, "<module>")) {
		...
		if (global != NULL) {
			...
			if (mod && check_string_obj(mod)) {
				xasprintf(&str_name, "<module:%s>", get_c_string(mod));
				needs_free = true;
			}
		}
	}
