...
COMMON_LDFLAGS := -lrt -ldl -pthread -Wl,-z,noexecstack $(LDFLAGS) $(LDFLAGS_$@)
ifneq ($(elfdir),)
...
_TARGETS += misc/demangler misc/symbols misc/dbginfo misc/uftrace_python.so
...
PYTHON_SRCS += $(srcdir)/utils/utils.c $(srcdir)/utils/rbtree.c
...
$(objdir)/misc/uftrace_python.so: $(PYTHON_OBJS)
	$(QUIET_LINK)$(CC) -shared $(PYTHON_CFLAGS) -o $@ $(PYTHON_OBJS) $(PYTHON_LDFLAGS)
else
$(objdir)/misc/uftrace_python.so:
endif
...
ifneq ($(findstring HAVE_LIBPYTHON, $(COMMON_CFLAGS)), )
	...
	$(Q)$(INSTALL) $(srcdir)/misc/uftrace.py  $(DESTDIR)$(libdir)/uftrace.py
	$(Q)$(INSTALL) $(objdir)/misc/uftrace_python.so  $(DESTDIR)$(libdir)/uftrace_python.so
endif
